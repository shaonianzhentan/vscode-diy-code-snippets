...
function getData(document: any, position: any, flags: string = '') {
    return new Promise((resolve, reject) => {
        let fileDir = document.fileName.substr(0, document.fileName.indexOf('\\src\\')) + "\\"
        // 读取配置
        let packagePath = `${fileDir}package.json`;
        if (!fs.existsSync(packagePath)) {
            // vscode.window.showInformationMessage('【自定义代码片段】配置文件不存在！');
            reject('【自定义代码片段】配置文件不存在！')
        }
        // 获取配置对象
        let packageConfig = JSON.parse(fs.readFileSync(packagePath, 'utf8'))
        // 判断配置节点是否存在
        if (!Reflect.has(packageConfig, 'dev-config')) {
            reject("没有开发配置节点")
        }
        let devConfig = packageConfig['dev-config']
        // 判断是否存在@types属性
        if (!Reflect.has(devConfig, '@types')) {
            reject("没有声明@types属性")
        }
        // 获取目录
        let dir = `${fileDir}${devConfig['@types']}`;
        ...
        try {
            ...
            if (result) {
                return result.map((ele: any) => {
                    ...
                    completionItem.kind = parseInt(vscode.CompletionItemKind[ele.type]);
